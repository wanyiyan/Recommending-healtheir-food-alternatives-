...
def train():
        ...
        for uid, bks, du in zip(uids, baskets, dynamic_user):
            ...
            for t, basket_t in enumerate(bks):
                if basket_t[0] != 0 and t != 0:
                    pos_idx = torch.cuda.LongTensor(basket_t) #torch.LongTensor(basket_t)

                    # Sample negative products
                    neg = random.sample(list(neg_samples[uid]), len(basket_t))
                    neg_idx = torch.cuda.LongTensor(neg) #torch.LongTensor(neg)
    ...
    def test_model():
        ...
        for i, x in enumerate(dh.batch_iter(train_data, Config().batch_size, Config().seq_len, shuffle=False)):
            ...
            for uid, l, du in zip(uids, lens, dynamic_user):
                ...
                positives = torch.cuda.LongTensor(positives) #torch.LongTensor(positives)
                ...
                negtives = torch.cuda.LongTensor(negtives) #torch.LongTensor(negtives)
    ...
    try:
        # Training
        for epoch in [102, 104, 106]: #range(100, Config().epochs):
            train_model()
            logger.info('-' * 89)
